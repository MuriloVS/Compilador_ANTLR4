...
//         int if_local = if_global;
//         if_global++;
//         //int else_local = else_global;
//         else_local.Add(else_global);
//         else_global++;        
//         Emit("NOT_IF_" + if_local, -2); 
//     }
//     OP_CUR ( statement )+ CL_CUR ( ELSE
//     {           
//         Emit("goto END_ELSE_" + else_local[else_local.Count - 1], 0);
//         System.Console.WriteLine("NOT_IF_" + if_local + ":");
//         has_else++;
//     }
//     OP_CUR ( statement )+ CL_CUR )? 
//     {
//         if (else_local.Count > 0)
//         {
//             System.Console.WriteLine("END_ELSE_" + else_local[else_local.Count - 1] + ":");  
//             else_local.RemoveAt(else_local.Count - 1);
//             //has_else--;
//         }
//         else
//         {
...
st_array_push: NAME
    {   
        if (!symbol_table.Contains($NAME.text)) {
            Console.Error.WriteLine("\nERROR - Variable does not existst_array_push' expression\n");         
            System.Environment.Exit(1);
        }
    ...
    DOT PUSH OP_PAR e = expression 
    {   
        if ($e.type != 'i') {
            Console.Error.WriteLine("\nERROR - Expression must evaluate to integer type - st_array_push' expression.\n");         
            System.Environment.Exit(1);            
        }
...
expression returns [char type]: t1 = term ( op = ( PLUS | MINUS ) t2 = term
    {
        if ($t1.type != 'i' || $t2.type != 'i') {
            Console.Error.WriteLine("t1 = " + $t1.type);
            Console.Error.WriteLine("t2 = " + $t2.type);
            Console.Error.WriteLine("\n# error: cannot mix types - plus or minus " + $t1.type + " " + $t2.type);         
            System.Environment.Exit(1);
        }
        if ($op.type == PLUS ) {
            Emit("iadd", -1);
...
factor returns [char type]: NUMBER
    ...
    | NAME OP_BRA NUMBER CL_BRA
    {
        ...
        Emit("aload "+ index, -1);
        Emit("ldc "+ index, -1);
        Emit("invokevirtual Array/get(I)I", 0);
        Emit("invokevirtual java/io/PrintStream/print(I)V", 0);
    };
...
/*factor: fa_number | fa_string | fa_expression | fa_name | fa_read_int | fa_read_str | fa_array_length;

fa_number returns [char type]: NUMBER
    {
        Emit("ldc " + $NUMBER.text, 1);
        $type = 'i';
    };

fa_string returns [char type]: STRING
    {
        Emit("ldc " + $STRING.text, 1);
        $type = 's';
    };

fa_expression returns [char type]: OP_PAR expression CL_PAR
    {
        $type = $expression.type;
    };

fa_name returns [char type]: NAME
    {
        if (!symbol_table.Contains($NAME.text)) {
            Console.Error.WriteLine("\nERROR - variable not found: '" + $NAME.text + "'\n");         
            System.Environment.Exit(1);
...
fa_read_int returns [char type]: READ_INT OP_PAR CL_PAR
    {
        Emit("invokestatic Runtime/readInt()I", 1);
        $type = 'i';
    };

fa_read_str returns [char type]: READ_STR OP_PAR CL_PAR
    {
        Emit("invokestatic Runtime/readString()Ljava/lang/String;", 1);        
        $type = 's';
    };

fa_array_length returns [char type]: NAME DOT LENGTH
    {
        int index = symbol_table.IndexOf($NAME.text); 
        char type = type_table[index];
        Emit("aload " + index, -1);
...
fa_array_get: NAME OP_BRA expression CL_BRA;
*/
