...
st_array_set: NAME OP_BRA e1 = expression CL_BRA ATTRIB e2 = expression;
...
expression returns [char type] : t1 = term ( op = ( PLUS | MINUS ) term
    {
        if ($t1.type != $term.type) {
...
term returns [char type]: f1 = factor ( op = ( TIMES | OVER | REM ) factor
    {
        if ($f1.type != $factor.type) {
